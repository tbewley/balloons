...
q=0.5
L(t)  = L0*(1+phi(t)*phidot(t)*C/Vs0(t)^q);                    % This is the control rule
...
syms phi phidot L Ldot phidotdot m g L0 C
% Vs0 = 0.5*m*L0^2*phidot^2 + m*g*L0*(1-cos(phi))
% L   = L0*(1+phi*phidot*C/Vs0^q), disp(' ')

disp('Ldot(phi,phidot) is now determined by solving it (symbolically) simultaneously with phidoddot,')
disp('as a set of two simultaneous equations:')
if q==1
...
elseif q==0.5
  eqn1 = Ldot == (C*L0^3*m*phidot^3*phidot + 2*C*L0^2*g*m*phi*phidotdot + 2*C*L0^2*g*m*phidot*phidot - 2*C*L0^2*g*m*cos(phi)*phi*phidotdot - 2*C*L0^2*g*m*cos(phi)*phidot*phidot - C*L0^2*g*m*sin(phi)*phi*phidot*phidot)/(2*(L0*g*m + (L0^2*m*phidot^2)/2 - L0*g*m*cos(phi))^(3/2))
end
eqn2 = phidotdot == -(g*sin(phi)+2*Ldot*phidot)/L;
q, SOL=solve(eqn1,eqn2,Ldot,phidotdot); Ldot=simplify(SOL.Ldot)
break
...
Lphi=1; Lphidot=3; N=40; eps=1e-9; Deltaphi=2*Lphi/(N-1); Deltaphidot=2*Lphidot/(N-1);
...
fig.Renderer='Painters'; figure(2); clf; surf(phi_grid,phidot_grid,Vs_grid),        title('Vs(phi,phidot)'),         xlabel('phi'), ylabel('phidot')
fig.Renderer='Painters'; figure(3); clf; surf(phi_grid,phidot_grid,Vsdot_grid),     title('Vsdot(phi,phidot)'),      xlabel('phi'), ylabel('phidot')
Vsdot_grid
